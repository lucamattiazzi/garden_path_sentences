"""Shared helpers for the qwen14b per-step scripts.

Mirror of qwen4b/scripts/_common.py, targeting `Qwen/Qwen3-14B-Base` —
the clean scale-up: same family, same Base pretraining recipe, same fp16
PyTorch stack as the 4B (unlike the 32B run, which was a post-trained
8-bit MLX model and therefore confounded).

The ablation/diagnostic libraries (`_ablation.py`, `_extra.py`) are reused
from qwen4b/scripts, which is appended to sys.path below. Appended, not
prepended: this folder's `_common` must keep winning name resolution.
"""
...
MODEL_SPEC = ModelSpec(hf_name="Qwen/Qwen3-14B-Base", approx_params="14B")

QWEN14B_DIR = Path(__file__).resolve().parents[1]
LOGS_DIR = QWEN14B_DIR / "logs"
